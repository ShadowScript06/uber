...
function RideOptionsPanel({ onBack, pickup, dropoff }) {
  const rides = [
    { name: "Uber Bike", price: "₹100", img: "/bike.jpg", capacity: 1 ,time:2},
    { name: "Uber Auto", price: "₹160", img: "/auto.jpg", capacity: 3 , time:6},
  ...
  return (<>
   {!showConfirmRide && (
     <div className={`absolute bottom-0 w-full h-[70vh] bg-white rounded-t-3xl shadow-lg p-5 flex flex-col
      transition-transform duration-500 ease-out transform ${mounted ? "translate-y-0" : "translate-y-full"}`}
   >
      {/* Top handle */}
      ...
      <div className="flex flex-col gap-3 overflow-auto flex-1">
        {rides.map((ride, idx) => (
          <div
            ...
            onClick={()=>handleSubmit(ride.price,ride.img)}
          >
   ...
   {showConfirmRide && (
        <ConfirmRidePanel pickup={pickup} dropoff={dropoff} fare={fare}  img={img} onBack={()=>setShowConfirmRide(false)}/>
   )}
   </>
  );
}
